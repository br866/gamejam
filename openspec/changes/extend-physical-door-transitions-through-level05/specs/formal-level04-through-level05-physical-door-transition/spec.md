...
### Requirement: Physical Level 5 arrival confirms the transition
The system SHALL mark Level 5 as current only after both players enter its existing arrival area beyond the preloaded L4.5-to-L5 shared door, then perform normal predecessor cleanup without repositioning players.

#### Scenario: Both players enter Level 5
- **WHEN** the preloaded Level 5 arrival area contains both players
- **THEN** Level 5 becomes the current formal level, neither player is repositioned, and normal Level 4.5 predecessor cleanup occurs
