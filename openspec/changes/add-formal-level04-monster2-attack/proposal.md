...
- **攻击机制改造（全局）**：所有使用 `MonsterPatrol` 的怪物的击杀流程从"接触即杀"改为"先攻击再杀"：
  - 追击中进入攻击距离后停止移动、面向玩家、播放攻击动画（前摇 windup）。
  - 前摇结束锁定必中：调用既有捕杀流程（`GameManager.OnPlayerCaught()`，L045 场景走 `FormalGameFlowController.ResetCurrentLevel()`）。
  - 目标进安全区/离开房间时取消攻击回巡逻，不破坏安全区规则。
  - `BeginForcedChase` 强制追杀路径同样走攻击流程；`ResetPatrol` 清理攻击状态。
- **动画驱动配合**：`MonsterAnimatorDriver` 新增状态锁接口，保证攻击动画在前摇期间不被速度判定切回 idle/walk。
- **各怪物配置攻击动画名**：L04 MonsterA/B → `monster1 find1`；L02 怪物 → `monster3 find`；新 Monster2 → `monster2 find`。
...
- `formal-monster-attack-catch`: 定义 MonsterPatrol 驱动的怪物"先攻击再捕杀"行为契约：攻击触发距离、前摇、锁定必中、安全区/出房取消、强制追杀路径一致性、状态复位，以及动画驱动器的状态锁要求。
...
- **脚本**：`Assets/MoMing/Scripts/Enemy/MonsterPatrol.cs`（新增 Attack 状态与字段）、`Assets/MoMing/Scripts/Enemy/MonsterAnimatorDriver.cs`（状态锁接口）。全局行为变更影响 Level02 与 L045 的怪物表现。
