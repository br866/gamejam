...
- [x] 1.2 `LevelMonsterNavigation.cs`: retry errored paths —in `Update`, when `currentPath != null && currentPath.error && Time.time >= nextRepathTime`, request a new path instead of early-returning forever.
...
- [x] 1.6 `LevelMonsterNavigation.cs`: door event handler —build contracted GUO over the collider bounds with `modifyWalkability`, `setWalkability = door.IsOpen`; queue it, call `AstarPath.FlushGraphUpdates()`, then force `RequestPath()`.
...
- [x] 1.9 `MonsterPatrol.cs`: in `Chase`, if `navigation.LastPathFailed` →clear target and return to `State.Patrol`.
...
- [x] 1.12 `LevelMonsterNavigation.cs`: establish an A* startup readiness gate: ensure `AstarPath` exists in `Awake`, defer `Seeker.StartPath` until the shared GridGraph is created and scanned, and issue any pending destination immediately after readiness.
...
- [x] 2.3 Verify statically that every navigation-owned `StartPath` is guarded by graph readiness and that startup destinations are retained until the first request.
...
- [x] 4.1 Open a door via GM key →right-click beyond it →monster paths through the doorway.
- [x] 4.2 Close that door while monster approaches →route cuts immediately during swing animation.
- [x] 4.3 Order monster into doorway then close →monster slides out within ~0.2 s.
- [x] 4.4 Force an unreachable target (all doors closed, click far side) →monster returns to waypoints and keeps moving; no freeze.
- [x] 4.5 Press F →monsters run along valid routes toward the marker, honoring walls and closed doors.
...
- [x] 5.2 Update task statuses; remind user of manual FormalLevel04 edits: enable flag on both monster instances, enlarge `areaCenter`/`areaSize` to cover the L4→L4.5 pursuit route, verify door blockingCollider references.
